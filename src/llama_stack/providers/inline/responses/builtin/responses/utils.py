    ...
    if isinstance(input, list):
        ...
        for i, input_item in enumerate(input):
            if isinstance(input_item, OpenAIResponseInputFunctionToolCallOutput):
                ...
                if reasoning:
                    assistant_msg = AssistantMessageWithReasoning(tool_calls=[tool_call], reasoning_content=reasoning)
                else:
                    assistant_msg = OpenAIAssistantMessageParam(tool_calls=[tool_call])
                ...
                reasoning = _get_preceding_reasoning(input, i)
                mcp_msg: OpenAIAssistantMessageParam
                if reasoning:
                    mcp_msg = AssistantMessageWithReasoning(tool_calls=[tool_call], reasoning_content=reasoning)
                else:
                    mcp_msg = OpenAIAssistantMessageParam(tool_calls=[tool_call])
...
def _get_preceding_reasoning(input_items: list[OpenAIResponseInput], index: int) -> str | None:
    ...
    if index > 0:
        ...
        if isinstance(item, OpenAIResponseOutputMessageReasoningItem) and item.content:
